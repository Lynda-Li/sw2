...
const actions: ActionTree<State, any> = {
  ...
  GET_TRACELIST(context: { state: State; commit: Commit }): Promise<void> {
    ...
    return graph
      ...
      .then((res: AxiosResponse) => {
        if (res.data.data.data.traces.length > 0) {
          res.data.data.data.traces.forEach((el: any, index: number) => {
            if (el.endpointNames[0] === '{POST}/**/*.bocms') {
              graph
              .query('queryTrace')
              .params({ traceId: el.traceIds[0] })
              .then((resQ: AxiosResponse) => {
                res.data.data.data.traces[index].spansTo = resQ.data.data.trace.spans[0].tags[0].value;
                res.data.data.data.traces[index].endpointNames = resQ.data.data.trace.spans[0].tags[0].value;
