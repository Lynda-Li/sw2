...
<template>
  <div class="rk-profile-header">
    <div>
      <a class="rk-new-task-btn r ml-10" @click="showDialogTaskCont">
        <span class="mr-5 vm">{{ this.$t('batchNewTask') }}</span>
      </a>
      <a class="rk-new-task-btn r ml-10" @click="showDialogTask">
      ...
      <rk-sidecont class="profile-task-box" width="100%" :title="this.$t('newTask')" :show.sync="dialogVisibleCont">
        <profileTaskBacthAdd :taskFieldSource="taskFieldSource" :newTaskFields="newTaskFields" @closeSideboxBacthAdd="closeSideboxBacthAdd" />
      </rk-sidecont>
...
<script lang="ts">
  ...
  import ProfileTaskBacthAdd from './profile-task-batchAdd.vue'
  
  @Component({ components: { TraceSelect, ProfileTask, ProfileTaskBacthAdd } })
  export default class ProfileHeader extends Vue {
    ...
    private dialogVisibleCont = false;
    ...
    private showDialogTaskCont() {
      this.dialogVisibleCont = true;
    }
    ...
    private closeSideboxBacthAdd() {
      this.dialogVisibleCont = false;
    }
  }
</script>
