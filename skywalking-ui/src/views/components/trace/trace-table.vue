      ...
      <table class="rk-trace-t">
        <tr class="rk-trace-tr cp" v-for="(i, index) in rocketTrace.traceList" @click="selectTrace(i)" :key="index">
          ...
          >
            ...
            >
              <span v-if="i.spansTo" class="b">{{ i.spansTo.split("http://")[1].substring(i.spansTo.split("http://")[1].indexOf('/')+1) }}</span>
              <span v-else class="b">{{ i.endpointNames[0] }}</span>
              <!-- <span class="b">{{ i.endpointNames[0] }}{{i.isError}}</span> -->
            </div>
